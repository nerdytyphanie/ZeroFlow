...
#if defined(ZEROFLOW_SERVER_APP)
constexpr auto kFixedRole = MainWindow::FixedRole::Server;
const auto kRoleName = QStringLiteral("ZeroFlow Server");
const auto kRoleId = QStringLiteral("zeroflow-server");
#elif defined(ZEROFLOW_CLIENT_APP)
constexpr auto kFixedRole = MainWindow::FixedRole::Client;
const auto kRoleName = QStringLiteral("ZeroFlow Client");
const auto kRoleId = QStringLiteral("zeroflow-client");
#else
#error "ZeroFlow GUI target must define a fixed server or client role"
#endif
...
int main(int argc, char *argv[])
{
  ...
  QCoreApplication::setApplicationName(kRoleName);
  ...
  QGuiApplication::setDesktopFileName(QStringLiteral("%1.%2").arg(kRevFqdnName, kRoleId));
  ...
  const auto shmId = QStringLiteral("%1-gui").arg(kRoleId);
  ...
  Settings::setValue(
      ...
      kFixedRole == MainWindow::FixedRole::Server ? Settings::CoreMode::Server : Settings::CoreMode::Client
  );
  Settings::setValue(Settings::Gui::AutoStartCore, true);
  Settings::setValue(Settings::Gui::Autohide, true);

  MainWindow mainWindow(kFixedRole);
  mainWindow.open();
