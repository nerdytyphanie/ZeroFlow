...
void CoreProcess::onProcessFinished(int exitCode, QProcess::ExitStatus)
{
  ...
  if (exitCode != s_exitSuccess) {
    ...
    if (wasStarted) {
      qWarning("desktop process failed while running, retrying in %d ms", kRetryDelay);
      ...
      [this](ProcessState state) {
        ...
        QTimer::singleShot(kRetryDelay, this, [this] {
          ...
          connect(m_coreIpcClient, &ipc::CoreIpcClient::connectionFailed, this, [this] {
            ...
            if (m_processState == ProcessState::Started && m_coreIpcClient) {
              QTimer::singleShot(kRetryDelay, m_coreIpcClient, [this] {
                if (m_processState == ProcessState::Started && m_coreIpcClient) {
