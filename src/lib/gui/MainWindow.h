...
public:
  explicit MainWindow(FixedRole role);
  ~MainWindow() override;
...
private:
  ...
  void announceLanServer();
  void expireLanServers();
  void readLanDiscovery();
  void refreshLanServerMenu();
  void selectLanServer(const QString &serverId);
  void setupLanDiscovery();
  ...
  const FixedRole m_fixedRole;
  const QString m_guiSocketName;
  ...
  QTimer *m_discoveryFallbackTimer = nullptr;
  QMenu *m_lanServerMenu = nullptr;
  struct LanServer
  {
    QString id;
    QString name;
  ...
  QHash<QString, LanServer> m_lanServers;
  QString m_selectedLanServerId;
  QString m_fallbackLanServerId;
