...
namespace { QString initialSettingsFile; bool settingsCreated = false; }
...
{
  if (!settingsCreated) { initialSettingsFile = settingsFile; instance(); return; }
...
Settings::Settings(QObject *parent) : QObject(parent), m_settingsWatcher{new QFileSystemWatcher(this)}
{
  ...
  QString fileToLoad;
  if (!initialSettingsFile.isEmpty()) fileToLoad = initialSettingsFile;
  else {
#ifdef Q_OS_WIN
  ...
  const auto stateFile = initialSettingsFile.isEmpty()
      ? QStringLiteral("%1/%2.state").arg(stateBase, kAppName)
      : QFileInfo(initialSettingsFile).absolutePath() + "/state.ini";
