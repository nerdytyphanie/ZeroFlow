...
void Server::adoptClient(BaseClientProxy *client)
{
  ...
  // Headless servers assign new peers to the next free edge without a GUI.
  // Persist with QSaveFile before accepting; an invalid layout never replaces the old file.
  if (!m_config->isScreen(client->getName())) {
    const auto name = client->getName();
    if (QString::fromStdString(name).contains(QRegularExpression("[^A-Za-z0-9_.-]")) || name.empty()) {
      closeClient(client, kMsgEUnknown); return;
    }
    // Parse into an unattached filter: copying a live Config also copies its
    // registered input hooks, whose destruction would remove the server hooks.
    ServerConfig next(m_events);
    QFile originalFile(Settings::serverConfigFile());
    if (!originalFile.open(QIODevice::ReadOnly)) { closeClient(client, kMsgEUnknown); return; }
    const auto original = QString::fromUtf8(originalFile.readAll());
    originalFile.close();
    std::istringstream snapshot(original.toStdString());
    snapshot >> next;
    std::string placedAnchor;
    Direction placedDirection = Direction::NoDirection;
    Direction placedOpposite = Direction::NoDirection;
    bool placed = false;
    for (auto screen = next.begin(); screen != next.end() && !placed; ++screen) {
      for (const auto direction : {Direction::Right, Direction::Left, Direction::Bottom, Direction::Top}) {
        if (next.hasNeighbor(*screen, direction)) continue;
        const auto anchor = *screen;
        const auto opposite = direction == Direction::Right ? Direction::Left :
          direction == Direction::Left ? Direction::Right : direction == Direction::Bottom ? Direction::Top : Direction::Bottom;
        placedAnchor = anchor; placedDirection = direction; placedOpposite = opposite;
        placed = next.addScreen(name) && next.connect(anchor, direction, 0, 1, name, 0, 1)
          && next.connect(name, opposite, 0, 1, anchor, 0, 1);
        break;
      }
    }
    std::ostringstream encoded;
    encoded << next;
    const auto generated = QString::fromStdString(encoded.str());
    // The legacy full serializer also writes GUI-derived options. Replace only
    // layout sections so clipboard settings, aliases and custom options survive.
    QString updated = original;
    for (const auto &section : {QStringLiteral("screens"), QStringLiteral("links")}) {
      const QRegularExpression expression(QStringLiteral("(?ms)^section:[ \t]*%1[ \t]*\r?\n.*?^end[ \t]*(?:\r?\n|$)").arg(section));
      const auto replacement = expression.match(generated).captured();
      if (replacement.isEmpty()) { closeClient(client, kMsgEUnknown); return; }
      const auto existing = expression.match(updated);
      if (existing.hasMatch()) updated.replace(existing.capturedStart(), existing.capturedLength(), replacement);
      else updated += "\n" + replacement;
    }
    const auto bytes = updated.toUtf8();
    QSaveFile file(Settings::serverConfigFile());
    if (!placed || !file.open(QIODevice::WriteOnly) || file.write(bytes.data(), bytes.size()) != qint64(bytes.size()) || !file.commit()) {
      LOG_ERR("cannot save automatic screen layout");
      closeClient(client, kMsgEUnknown); return;
    }
    // Apply geometry only; the live input filter and its registered hotkeys stay intact.
...
void Server::onClipboardChanged(const BaseClientProxy *sender, ClipboardID id, uint32_t seqNum)
{
  ...
  if (data.size() > m_maximumClipboardSize) {
    LOG_WARN("not sending clipboard data, exceeds limit: %zu bytes", m_maximumClipboardSize);
    return;
  }
...
{
  // A pending destination may disconnect before it becomes the active screen.
  ...
  m_primaryClient->releaseMouseButtons();
