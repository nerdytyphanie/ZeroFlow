...
{
  m_buffer->init();
  auto *heartbeat = newTimer(2.0, nullptr);
  addHandler(EventTypes::Timer, heartbeat, [](const auto &) { HeadlessStatus::heartbeat(); });
  HeadlessStatus::heartbeat();
  {
  ...
  removeHandlers(heartbeat);
  deleteTimer(heartbeat);
  return exitCode;
}
