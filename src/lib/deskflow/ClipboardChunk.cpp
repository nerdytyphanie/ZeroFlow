...
)
{
  ...
  if (mark == ChunkType::DataStart) {
    ...
    if (state.discarding) {
      LOG_WARN("skipping oversized clipboard, size: %zu, limit: %zu", state.expectedSize, maxDataSize);
      // Consume the transfer without retaining its contents or interrupting input.
      return InProgress;
    }
    ...
    if (state.discarding) return InProgress;
