...
void Screen::releaseMouseButtons()
{
  uint32_t button = kButtonNone;
  if (!m_screen->isAnyMouseButtonDown(button)) return;

  LOG_DEBUG("releasing held mouse buttons after disconnect");
  if (m_isPrimary) {
    m_screen->fakeInputBegin();
  }
  // An unsolicited button-up can activate a control or open a context menu.
  // Use the platform's existing button state; an idle reset injects no input.
  for (int released = 0; released < 5 && m_screen->isAnyMouseButtonDown(button); ++released) {
    if (button < kButtonLeft || button > kButtonExtra1) break;
    m_screen->fakeMouseButton(static_cast<ButtonID>(button), false);
  }
  if (m_isPrimary) {
