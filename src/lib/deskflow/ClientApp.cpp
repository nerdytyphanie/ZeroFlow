...
void ClientApp::handleClientConnected()
{
  ...
  const auto selectedServerId = Settings::value(Settings::Client::SelectedServerId).toString();
  if (!selectedServerId.isEmpty()) {
    Settings::setValue(Settings::Client::LastServerId, selectedServerId);
    Settings::save(false);
  }
...
int ClientApp::mainLoop()
{
  ...
  stopClient();
  // Join the socket worker before its creator (the core thread) exits.
  setSocketMultiplexer({});
