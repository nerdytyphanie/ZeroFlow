...
bool SecureSocket::verifyCertFingerprint(const QString &FingerprintDatabasePath) const
{
  ...
  static std::mutex trustGate;
  std::scoped_lock trustLock(trustGate);
  ...
  if (!db.isTrusted(sha256)) {
    // Match ZeroFlow's existing automatic acceptance policy without a GUI.
    if (!QDir().mkpath(QFileInfo(path).absolutePath())) return false;
    db.addTrusted(sha256);
    if (!db.write(path)) { LOG_ERR("cannot persist accepted peer fingerprint"); return false; }
    LOG_INFO("automatically accepted TLS peer fingerprint");
  }
